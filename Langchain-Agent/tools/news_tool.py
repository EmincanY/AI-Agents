...
class NewsInput(BaseModel):
    query: str = Field(description="The topic or search query for news articles")
    count: int = Field(default=5, description="Number of headlines to return (default: 5)")

class NewsTool(BaseTool):
    name: str = "get_news_headlines"
    description: str = "Get latest news headlines about a specific topic"
    ...
    def _run(self, query: str, count: int = 5) -> str:
        ...
        try:
            ...
            search_strategies = [
                {
                    'query': f'"{query}"',  # Exact phrase match
                    'relevance': 'high'
                },
                {
                ...
                {
                    'query': f"{query} latest",  # Latest news
                    'relevance': 'medium'
                }
            ]
            ...
            seen_titles = set()
            required_keywords = set(query.lower().split())
            ...
            for strategy in search_strategies:
                ...
                try:
                    news = newsapi.get_everything(
                        ...
                        language='en',
                        sort_by='relevancy',
                        page_size=min(50, remaining_count * 5)
                    )
            ...
            return f"No relevant news found for topic: {query}"
        except Exception as e:
            return f"Error fetching news: {str(e)}"

    async def _arun(self, query: str, count: int = 5) -> str:
        """Async implementation of news headlines"""
